...
def get_config_file_path(default_env: str = "PRODUCTION") -> str:
    ...
    if getattr(sys, "frozen", False) and hasattr(sys, "_MEIPASS"):
        ...
        # First check project root directory (selfhost/)
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../.."))
        root_config_path = os.path.join(project_root, config_file)
        if os.path.exists(root_config_path):
            return root_config_path
        # Fall back to helpers directory for backward compatibility
...
AUTH_ENV_FILE = "services.auth.env.AUTH_ENV_FILE"
...
NEXT_PUBLIC_AUTH_URL = "services.view.env.NEXT_PUBLIC_AUTH_URL"
...
BETTER_AUTH_URL = "services.api.env.BETTER_AUTH_URL"
BETTER_AUTH_SECRET = "services.api.env.BETTER_AUTH_SECRET"
