...
from app.utils.config import API_ENV_FILE, CADDY_CONFIG_VOLUME, NIXOPUS_CONFIG_DIR, VIEW_ENV_FILE, get_config_value
...
def rollback_auth_service_env_file(
    config_resolver: ConfigResolver,
    dry_run: bool,
    logger: Optional[LoggerProtocol] = None,
) -> Tuple[bool, Optional[str]]:
    """Rollback auth .env file"""
    try:
        nixopus_config_dir = config_resolver.get(NIXOPUS_CONFIG_DIR)
        auth_env_file = os.path.join(nixopus_config_dir, "auth", ".env")
        
        dry_run_result = handle_dry_run(f"Would remove auth .env file: {auth_env_file}", dry_run, logger)
        if dry_run_result[0]:
            return dry_run_result
        
        if auth_env_file and isinstance(auth_env_file, str) and auth_env_file.strip():
            safe_remove_file(
                auth_env_file,
                logger,
                f"Removed auth env file: {auth_env_file}",
            )
        
        return True, None
    except Exception as e:
        error_msg = f"Failed to remove auth .env file: {str(e)}"
        if logger:
            logger.warning(error_msg)
        return False, error_msg
...
def build_rollback_functions(
    ...
    return {
        ...
        "Cloning auth repository": lambda: rollback_repository(
            os.path.join(config_resolver.get(NIXOPUS_CONFIG_DIR), "auth"), dry_run, logger
        ),
        "Creating auth environment file": lambda: rollback_auth_service_env_file(
            config_resolver, dry_run, logger
        ),
    }
