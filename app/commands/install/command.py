...
def install_callback(
    ...
    api_domain: str = typer.Option(
        ...
        "--api-domain",
        "-ad",
        help="The domain where the nixopus api will be accessible (e.g. api.nixopus.com), if not provided Caddy will handle routing via HTTP/HTTPS ports",
    ),
    view_domain: str = typer.Option(
        None,
        "--view-domain",
        "-vd",
        help="The domain where the nixopus view will be accessible (e.g. nixopus.com), if not provided Caddy will handle routing via HTTP/HTTPS ports",
    ),
    ...
    api_port: int = typer.Option(None, "--api-port", help="External port for API service when using IP addresses (default: 8443)"),
    view_port: int = typer.Option(None, "--view-port", help="External port for View service when using IP addresses (default: 7443)"),
    auth_port: int = typer.Option(None, "--auth-port", help="External port for Auth service when using IP addresses (default: 9090)"),
    db_port: int = typer.Option(None, "--db-port", help="External port for PostgreSQL database when using IP addresses (default: 5432)"),
    redis_port: int = typer.Option(None, "--redis-port", help="External port for Redis when using IP addresses (default: 6379)"),
    better_auth_url: str = typer.Option(None, "--better-auth-url", help="Better Auth URL (defaults to config value)"),
    better_auth_secret: str = typer.Option(None, "--better-auth-secret", help="Better Auth secret key (defaults to config value)"),
    ...
    resend_api_key: Optional[str] = typer.Option(None, "--resend-api-key", help="Resend API key for email sending (optional, defaults to empty if not provided)"),
    resend_from_email: Optional[str] = typer.Option(None, "--resend-from-email", help="Resend from email address (optional, defaults to empty if not provided)"),
):
