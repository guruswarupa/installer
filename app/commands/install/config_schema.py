...
PORT_MAPPINGS = [
    ...
    PortMapping("API_PORT", "api_port", "External port for API service"),
    PortMapping("NEXT_PUBLIC_PORT", "view_port", "External port for View service"),
    PortMapping("AUTH_SERVICE_PORT", "auth_port", "External port for Auth service"),
    PortMapping("DB_PORT", "db_port", "External port for PostgreSQL database"),
    PortMapping("REDIS_PORT", "redis_port", "External port for Redis"),
]
...
ENV_VAR_KEYS = {
    "ALLOWED_ORIGIN": EnvVarDefinition(
        ...
        computed=True,
        depends_on=["VIEW_DOMAIN", "NEXT_PUBLIC_PORT"],
    ),
    ...
    "CORS_ALLOWED_ORIGINS": EnvVarDefinition(
        key="CORS_ALLOWED_ORIGINS",
        var_type=EnvVarType.STRING,
        description="Better Auth CORS allowed origins (comma-separated list of view and API domains)",
        computed=True,
        depends_on=["VIEW_DOMAIN", "API_DOMAIN", "NEXT_PUBLIC_PORT", "API_PORT", "AUTH_SERVICE_PORT"],
    ),
    "BETTER_AUTH_URL": EnvVarDefinition(
        key="BETTER_AUTH_URL",
        var_type=EnvVarType.URL,
        description="Better Auth URL",
        computed=True,
        depends_on=["API_DOMAIN", "AUTH_SERVICE_PORT"],
    ),
    "BETTER_AUTH_SECRET": EnvVarDefinition(
        key="BETTER_AUTH_SECRET",
        var_type=EnvVarType.STRING,
        description="Better Auth secret key (override in production)",
        computed=False,
        default=os.environ.get("BETTER_AUTH_SECRET", "NixopusBetterAuthSecretKey"),
    ),
    "NEXT_PUBLIC_AUTH_URL": EnvVarDefinition(
        key="NEXT_PUBLIC_AUTH_URL",
        var_type=EnvVarType.URL,
        description="Next.js public auth URL (frontend auth service URL)",
        computed=True,
        depends_on=["BETTER_AUTH_URL"],
    ),
}
