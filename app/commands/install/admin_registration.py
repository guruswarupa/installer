...
def get_api_base_url(api_domain: Optional[str], host_ip: str, caddy_http_port: Optional[int] = None) -> str:
    """Get API base URL using Caddy ports (external access)."""
    secure = api_domain is not None
    if secure:
        api_host = api_domain
        protocol = "https"
    else:
        http_port = get_caddy_http_port(caddy_http_port)
        api_host = f"{host_ip}:{http_port}"
        protocol = "http"
    return f"{protocol}://{api_host}"


def get_caddy_http_port(caddy_http_port: Optional[int]) -> int:
    """Get Caddy HTTP port (default: 80)."""
    return caddy_http_port if caddy_http_port is not None else 80
...
def build_api_base_url(config_resolver: ConfigResolver, params: InstallParams) -> str:
    """Build API base URL using Caddy ports for external access."""
    host_ip = get_host_ip_or_default(params.host_ip)
    return get_api_base_url(params.api_domain, host_ip, params.caddy_http_port)
