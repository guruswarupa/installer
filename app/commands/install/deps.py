    ...
    def check_dependency(self, dep, package_manager):
        try:
            if dep["command"]:
                ...
                if shutil.which(dep["command"]) is not None:
                    return True
                # If not in PATH, check common system directories
                # (some commands like sshd are in /usr/sbin which may not be in PATH)
                system_paths = ["/usr/sbin", "/usr/bin", "/sbin", "/bin", "/usr/local/sbin", "/usr/local/bin"]
                for path in system_paths:
                    full_path = f"{path}/{dep['command']}"
